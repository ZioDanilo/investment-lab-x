...
export const encodeTransportBatch = (paths: unknown[] = []): {
  message: { arrays: { annualEtfDeviationCode: Uint32Array; payload: ArrayBuffer } };
  transferList: ArrayBuffer[];
  paths?: undefined;
} => {
  const normalizedPaths = Array.isArray(paths) ? paths : [];
  const annualEtfDeviationCode: number[] = [];

  for (const path of normalizedPaths as any[]) {
...
export const decodeTransportBatch = (transport: any): any[] => {
