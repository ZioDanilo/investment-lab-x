...
export const normalQuantile = (probability) => {
...
export const sampleStandardNormal = (random) => {
    const first = random();
    const second = random();
    assertUniform(first);
...
export const studentTCdf = (value) => {
...
export const studentTQuantile = (probability) => {
    assertProbability(probability);
    if (probability === 0.5)
        return 0;
    let lower = -1;
    let upper = 1;
    while (studentTCdf(lower) > probability)
        lower *= 2;
    while (studentTCdf(upper) < probability)
        upper *= 2;
...
export const sampleChiSquare5 = (random) => {
...
export const sampleStandardizedStudentT5 = (random) => {
    const normal = sampleStandardNormal(random);
...
export const truncatedNormalCdf = (value, parameters) => {
...
export const truncatedNormalQuantile = (probability, parameters) => {
    assertTruncatedNormalParameters(parameters);
    assertProbability(probability);
    const { mean, standardDeviation, lower, upper } = parameters;
    const lowerProbability = normalCdf((lower - mean) / standardDeviation);
...
export const sampleTruncatedNormal = (parameters, random) => {
    const uniform = random();
    assertUniform(uniform);
...
export const solveTruncatedNormalMeanForQuantile = ({ mean, standardDeviation, lower, upper, quantile, targetValue }) => {
    ...
    if (truncatedNormalQuantile(quantile, baseParameters) <= targetValue)
        return mean;
    let upperMean = mean;
    ...
    for (let iteration = 0; iteration < INTENSITY_MEAN_SOLVER_MAX_ITERATIONS; iteration += 1) {
        ...
        if (Math.abs(currentQuantile - targetValue) <= INTENSITY_MEAN_SOLVER_TOLERANCE)
            return midpoint;
