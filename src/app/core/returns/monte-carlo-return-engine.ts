import { MonteCarloScenario, MonteCarloSnapshot } from '../models/monte-carlo-contracts.model';
import { MonteCarloPrecomputation, PreparedEtfScenarioParameters } from '../precomputation/monte-carlo-precomputation';
import {
  COPULA_EPSILON,
  sampleChiSquare5,
...
export interface EffectiveMonthlyParameters {
  ...
  scenarioMonthlyReturnRange: { min: number; max: number };
  reconstructedReturnRange: { min: number; max: number };
  effectiveReturnRange: { min: number; max: number };
}

export interface MonthlyEtfReturn {
  isin: string;
  effectiveParameters: EffectiveMonthlyParameters;
  standardizedShock: number;
  monthlyReturn: number;
}

export interface MonthlyReturnRangeDiagnostics {
...
export interface MonthlyReturnDiagnostics {
  attempts: number;
  rejectedAttempts: number;
  acceptedChiSquare: number;
  rejectedChiSquares: number[];
  targetCorrelation: number[][];
  operationalCorrelation: number[][];
  latentCorrelation: number[][];
  rangeDiagnostics?: MonthlyReturnRangeDiagnostics;
}

export interface MonthlyReturnVector {
  scenario: MonteCarloScenario;
  intensity: number;
  etfReturns: MonthlyEtfReturn[];
  diagnostics: MonthlyReturnDiagnostics;
}

export interface EmpiricalCorrelationDiagnostics {
  sampleSize: number;
  targetCorrelation: number[][];
  operationalCorrelation: number[][];
  latentCorrelation: number[][];
  empiricalShockCorrelation: number[][] | null;
  empiricalReturnCorrelation: number[][] | null;
  lowerTailDependence: number[][] | null;
  upperTailDependence: number[][] | null;
}
...
export const __debugRejectTrace: Array<{
  scenario: MonteCarloScenario;
  vectorNumber: number;
...
export const __debugAcceptTrace: Array<{
  scenario: MonteCarloScenario;
  vectorNumber: number;
  attemptNumber: number;
  isin: string;
...
export const __debugAllAttemptTrace: Array<{
  scenario: MonteCarloScenario;
  intensity: number;
  attemptNumber: number;
  index: number;
  ...
  standardizedShock: number;
  monthlyReturn: number;
  acceptedForThisEtf: boolean;
  vectorAccepted: boolean;
}> = [];
...
const lookupCalibratedMuFromIntensity = (
  parameters: PreparedEtfScenarioParameters & { generalMonthlyExpectedReturn?: number; generalMonthlyVolatility?: number },
  intensity: number
): number => {
  ...
  if (clampedIntensity === 0) return calibrationCurve[0].muMonthly;
  if (clampedIntensity === 1) return calibrationCurve[calibrationCurve.length - 1].muMonthly;
...
export const calculateEffectiveMonthlyParameters = (
  parameters: PreparedEtfScenarioParameters & { generalMonthlyExpectedReturn?: number; generalMonthlyVolatility?: number },
  intensity: number,
  isin: string
): EffectiveMonthlyParameters => {
...
export const isMonthlyReturnAccepted = (monthlyReturn: number, effectiveReturnRange: { min: number; max: number }): boolean =>
  Number.isFinite(monthlyReturn) && monthlyReturn >= -1;

const calculateCorrelation = (samples: number[][]): number[][] | null => {
  if (samples.length < 2) return null;
  const dimension = samples[0].length;
  const means = Array<number>(dimension).fill(0);
  for (const sample of samples) {
    for (let index = 0; index < dimension; index += 1) means[index] += sample[index];
  }
  for (let index = 0; index < dimension; index += 1) means[index] /= samples.length;
  ...
  for (let row = 0; row < dimension; row += 1) {
    for (let column = 0; column < dimension; column += 1) covariance[row][column] /= samples.length - 1;
  }
...
export class ReturnCorrelationDiagnosticsAccumulator {
  private readonly shocks: number[][] = [];
  private readonly returns: number[][] = [];

  constructor(
    private readonly targetCorrelation: number[][],
    private readonly operationalCorrelation: number[][]
  ) {}

  record(vector: MonthlyReturnVector): void {
  ...
  toDiagnostics(): EmpiricalCorrelationDiagnostics {
    return {
...
export const generateMonthlyReturnVector = (
  ...
  if (!matrixPreparation) fail('MISSING_CORRELATION_MATRIX', 'precomputed scenario matrix is required', { scenario });
  ...
  const effectiveParameters = assetIsins.map((isin) => {
    ...
    if (!parameters) fail('MISSING_PRECOMPUTED_ETF_PARAMETERS', 'missing precomputed ETF parameters', { isin, scenario });
    return calculateEffectiveMonthlyParameters(parameters, intensity, isin);
  });

  const rangeBucketState: Record<string, {
    candidateReturnCount: number;
    belowEffectiveMinCount: number;
    aboveEffectiveMaxCount: number;
    lowerDistanceSigmaSum: number;
  ...
  for (let attempt = 1; attempt <= MAX_REDRAWS; attempt += 1) {
    ...
    const standardizedShocks = correlatedNormals.map((normal) => {
      ...
      if (!Number.isFinite(shock)) fail('INVALID_STUDENT_T_SHOCK', 't-copula produced a non-finite standardized shock', { scenario, attempt });
      return shock;
    });
