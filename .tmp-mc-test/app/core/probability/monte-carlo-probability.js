...
exports.solveTruncatedNormalMeanForQuantile = exports.sampleTruncatedNormal = exports.truncatedNormalQuantile = exports.truncatedNormalCdf = exports.sampleStandardizedStudentT5 = exports.sampleChiSquare5 = exports.studentTQuantile = exports.studentTCdf = exports.sampleStandardNormal = exports.normalQuantile = exports.normalCdf = exports.MonteCarloProbabilityError = exports.INTENSITY_MEAN_SOLVER_MAX_ITERATIONS = exports.INTENSITY_MEAN_SOLVER_TOLERANCE = exports.COPULA_EPSILON = exports.STUDENT_T_STANDARDIZATION = exports.T_COPULA_DOF = void 0;
exports.T_COPULA_DOF = 5;
exports.STUDENT_T_STANDARDIZATION = Math.sqrt(3 / 5);
exports.COPULA_EPSILON = 1e-12;
exports.INTENSITY_MEAN_SOLVER_TOLERANCE = 1e-10;
exports.INTENSITY_MEAN_SOLVER_MAX_ITERATIONS = 100;
const QUANTILE_TOLERANCE = 1e-12;
const QUANTILE_MAX_ITERATIONS = 200;
class MonteCarloProbabilityError extends Error {
    code;
    details;
...
const assertTruncatedNormalParameters = ({ mean, standardDeviation, lower, upper }) => {
...
const normalQuantile = (probability) => {
...
exports.sampleStandardNormal = sampleStandardNormal;
...
const regularizedIncompleteBeta = (value, alpha, beta) => {
    if (value <= 0)
        return 0;
    if (value >= 1)
        return 1;
...
const studentTCdf = (value) => {
    assertFinite(value, 'Student-t value');
    const degreesOfFreedom = exports.T_COPULA_DOF;
...
const studentTQuantile = (probability) => {
    assertProbability(probability);
    if (probability === 0.5)
        return 0;
    let lower = -1;
    let upper = 1;
    while ((0, exports.studentTCdf)(lower) > probability)
        lower *= 2;
    while ((0, exports.studentTCdf)(upper) < probability)
        upper *= 2;
    for (let iteration = 0; iteration < QUANTILE_MAX_ITERATIONS; iteration += 1) {
        const midpoint = (lower + upper) / 2;
        const midpointProbability = (0, exports.studentTCdf)(midpoint);
        if (Math.abs(midpointProbability - probability) <= QUANTILE_TOLERANCE)
            return midpoint;
        if (midpointProbability < probability)
            lower = midpoint;
        else
            upper = midpoint;
    }
...
const sampleGamma = (shape, random) => {
    if (shape <= 0 || !Number.isFinite(shape))
        fail('INVALID_GAMMA_SHAPE', 'gamma shape must be finite and greater than zero', { shape });
    if (shape < 1) {
    ...
    while (true) {
        const normal = (0, exports.sampleStandardNormal)(random);
...
const sampleChiSquare5 = (random) => {
    const sample = 2 * sampleGamma(exports.T_COPULA_DOF / 2, random);
    if (!Number.isFinite(sample) || sample <= 0)
        fail('INVALID_CHI_SQUARE_SAMPLE', 'chi-square sample must be finite and positive', { sample });
    return sample;
};
exports.sampleChiSquare5 = sampleChiSquare5;
const sampleStandardizedStudentT5 = (random) => {
    const normal = (0, exports.sampleStandardNormal)(random);
    const chiSquare = (0, exports.sampleChiSquare5)(random);
    const sample = normal / Math.sqrt(chiSquare / exports.T_COPULA_DOF) * exports.STUDENT_T_STANDARDIZATION;
    assertFinite(sample, 'standardized Student-t sample');
    return sample;
};
exports.sampleStandardizedStudentT5 = sampleStandardizedStudentT5;
const truncatedNormalCdf = (value, parameters) => {
    ...
    if (value >= upper)
        return 1;
    const lowerProbability = (0, exports.normalCdf)((lower - mean) / standardDeviation);
    const upperProbability = (0, exports.normalCdf)((upper - mean) / standardDeviation);
    ...
    return ((0, exports.normalCdf)((value - mean) / standardDeviation) - lowerProbability) / normalization;
};
exports.truncatedNormalCdf = truncatedNormalCdf;
const truncatedNormalQuantile = (probability, parameters) => {
    assertTruncatedNormalParameters(parameters);
    assertProbability(probability);
    const { mean, standardDeviation, lower, upper } = parameters;
    const lowerProbability = (0, exports.normalCdf)((lower - mean) / standardDeviation);
    ...
    const quantile = mean + standardDeviation * (0, exports.normalQuantile)(lowerProbability + probability * normalization);
...
exports.truncatedNormalQuantile = truncatedNormalQuantile;
const sampleTruncatedNormal = (parameters, random) => {
    const uniform = random();
    assertUniform(uniform);
    const sample = (0, exports.truncatedNormalQuantile)(uniform === 0 ? Number.MIN_VALUE : uniform, parameters);
...
exports.sampleTruncatedNormal = sampleTruncatedNormal;
const solveTruncatedNormalMeanForQuantile = ({ mean, standardDeviation, lower, upper, quantile, targetValue }) => {
    ...
    if ((0, exports.truncatedNormalQuantile)(quantile, baseParameters) <= targetValue)
        return mean;
    let upperMean = mean;
    let lowerMean = mean - standardDeviation;
    let step = standardDeviation;
    for (let iteration = 0; (0, exports.truncatedNormalQuantile)(quantile, { mean: lowerMean, standardDeviation, lower, upper }) > targetValue; iteration += 1) {
        if (iteration >= exports.INTENSITY_MEAN_SOLVER_MAX_ITERATIONS) {
    ...
    for (let iteration = 0; iteration < exports.INTENSITY_MEAN_SOLVER_MAX_ITERATIONS; iteration += 1) {
        const midpoint = (lowerMean + upperMean) / 2;
        const currentQuantile = (0, exports.truncatedNormalQuantile)(quantile, { mean: midpoint, standardDeviation, lower, upper });
        if (Math.abs(currentQuantile - targetValue) <= exports.INTENSITY_MEAN_SOLVER_TOLERANCE)
            return midpoint;
        if (currentQuantile > targetValue)
            upperMean = midpoint;
        else
            lowerMean = midpoint;
    }
...
exports.solveTruncatedNormalMeanForQuantile = solveTruncatedNormalMeanForQuantile;
