...
exports.generateMonthlyReturnVector = exports.ReturnCorrelationDiagnosticsAccumulator = exports.isMonthlyReturnAccepted = exports.calculateEffectiveMonthlyParameters = exports.__debugAllAttemptTrace = exports.__debugAcceptTrace = exports.__debugRejectTrace = exports.MonteCarloReturnEngineError = exports.MAX_REDRAWS = void 0;
const monte_carlo_probability_1 = require("../probability/monte-carlo-probability");
exports.MAX_REDRAWS = 1000;
class MonteCarloReturnEngineError extends Error {
    code;
    details;
...
exports.MonteCarloReturnEngineError = MonteCarloReturnEngineError;
exports.__debugRejectTrace = [];
exports.__debugAcceptTrace = [];
exports.__debugAllAttemptTrace = [];
...
const clampCopulaProbability = (probability) => {
    ...
    return Math.min(1 - monte_carlo_probability_1.COPULA_EPSILON, Math.max(monte_carlo_probability_1.COPULA_EPSILON, probability));
};
...
const calculateEffectiveMonthlyParameters = (parameters, intensity, isin) => {
...
exports.calculateEffectiveMonthlyParameters = calculateEffectiveMonthlyParameters;
const isMonthlyReturnAccepted = (monthlyReturn, effectiveReturnRange) => Number.isFinite(monthlyReturn) && monthlyReturn >= -1;
exports.isMonthlyReturnAccepted = isMonthlyReturnAccepted;
...
class ReturnCorrelationDiagnosticsAccumulator {
    targetCorrelation;
...
exports.ReturnCorrelationDiagnosticsAccumulator = ReturnCorrelationDiagnosticsAccumulator;
let debugVectorCounter = 0;
const generateMonthlyReturnVector = (snapshot, precomputation, scenario, intensity, random) => {
    ...
    const effectiveParameters = assetIsins.map((isin) => {
        ...
        return (0, exports.calculateEffectiveMonthlyParameters)(parameters, intensity, isin);
    });
    ...
    for (let attempt = 1; attempt <= exports.MAX_REDRAWS; attempt += 1) {
        const independentNormals = assetIsins.map(() => (0, monte_carlo_probability_1.sampleStandardNormal)(random));
        const correlatedNormals = multiplyMatrixVector(factor, independentNormals);
        const commonChiSquare = (0, monte_carlo_probability_1.sampleChiSquare5)(random);
        const standardizedShocks = correlatedNormals.map((normal) => {
            const correlatedT = normal / Math.sqrt(commonChiSquare / 5);
            const probability = clampCopulaProbability((0, monte_carlo_probability_1.studentTCdf)(correlatedT));
            const shock = (0, monte_carlo_probability_1.studentTQuantile)(probability) * monte_carlo_probability_1.STUDENT_T_STANDARDIZATION;
        ...
        const perEtfStatus = assetIsins.map((isin, index) => {
            ...
            const accepted = (0, exports.isMonthlyReturnAccepted)(candidateReturn, parameters.effectiveReturnRange);
        ...
        for (const entry of perEtfStatus) {
            ...
            exports.__debugAllAttemptTrace.push(attemptRecord);
        }
        ...
        if (debugRejectEnabled) {
            for (const entry of perEtfStatus) {
                exports.__debugAcceptTrace.push({
                    scenario,
    ...
    return fail('MAX_REDRAWS_EXCEEDED', 'no valid monthly return vector was generated within MAX_REDRAWS', {
        scenario,
        intensity,
        maxRedraws: exports.MAX_REDRAWS,
        assetIsins
    });
};
exports.generateMonthlyReturnVector = generateMonthlyReturnVector;
