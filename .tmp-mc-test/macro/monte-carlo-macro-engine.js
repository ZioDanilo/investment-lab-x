...
exports.macroEngineConstants = exports.generateMonthlyMacroTimeline = exports.getStayProbability = exports.MonteCarloMacroEngineError = exports.ENTRY_SOFT_QUANTILE = exports.INTENSITY_AR_RHO = void 0;
const monte_carlo_probability_1 = require("../probability/monte-carlo-probability");
exports.INTENSITY_AR_RHO = 0.85;
exports.ENTRY_SOFT_QUANTILE = 0.95;
class MonteCarloMacroEngineError extends Error {
    code;
    details;
...
exports.MonteCarloMacroEngineError = MonteCarloMacroEngineError;
...
const getStayDecision = (scenario, intensity, monthsInCurrentScenario, snapshot, random) => {
    ...
    const { probability, cause } = (0, exports.getStayProbability)(configuration, monthsInCurrentScenario);
    const draw = random();
    if (!Number.isFinite(draw) || draw < 0 || draw >= 1) {
        fail('INVALID_UNIFORM_RANDOM_VALUE', 'uniform random source must return a value in [0, 1)', { draw });
    }
...
const getStayProbability = (configuration, monthsInCurrentScenario) => {
...
exports.getStayProbability = getStayProbability;
...
const createAr1IntensityParameters = (configuration, previousIntensity, maxMonthlyVariation) => ({
    mean: configuration.meanIntensity + exports.INTENSITY_AR_RHO * (previousIntensity - configuration.meanIntensity),
    standardDeviation: configuration.stdDevIntensity * Math.sqrt(1 - exports.INTENSITY_AR_RHO ** 2),
...
const applySoftThreshold = (parameters, threshold) => {
    if ((0, monte_carlo_probability_1.truncatedNormalQuantile)(exports.ENTRY_SOFT_QUANTILE, parameters) <= threshold)
        return parameters;
    const mean = (0, monte_carlo_probability_1.solveTruncatedNormalMeanForQuantile)({
        ...parameters,
        quantile: exports.ENTRY_SOFT_QUANTILE,
        targetValue: threshold
    });
    return { ...parameters, mean };
};
const sampleIntensity = (parameters, kind, softThreshold, random) => {
    const intensity = (0, monte_carlo_probability_1.sampleTruncatedNormal)(parameters, random);
...
const generateMonthlyMacroTimeline = (snapshot, numberOfMonths, random) => {
...
exports.generateMonthlyMacroTimeline = generateMonthlyMacroTimeline;
exports.macroEngineConstants = {
    INTENSITY_AR_RHO: exports.INTENSITY_AR_RHO,
    ENTRY_SOFT_QUANTILE: exports.ENTRY_SOFT_QUANTILE,
    INTENSITY_MEAN_SOLVER_TOLERANCE: monte_carlo_probability_1.INTENSITY_MEAN_SOLVER_TOLERANCE,
    INTENSITY_MEAN_SOLVER_MAX_ITERATIONS: monte_carlo_probability_1.INTENSITY_MEAN_SOLVER_MAX_ITERATIONS
};
