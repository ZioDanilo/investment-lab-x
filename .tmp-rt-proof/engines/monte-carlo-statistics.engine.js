...
const getAdvTimingStore = () => {
    const scope = globalThis;
    if (!Array.isArray(scope[ADV_TIMING_KEY])) {
        scope[ADV_TIMING_KEY] = [];
    }
    return scope[ADV_TIMING_KEY];
};
const recordAdvTiming = (operation, scope, startMs, endMs, sampleCount, dimension) => {
...
const buildDeltaMatrix = (empiricalReturn, target, absolute = false) => {
...
class MonteCarloStatisticsEngine {
    static DRAWDOWN_REFERENCE_YEARS = 30;
    static DRAWDOWN_HORIZON_EXPONENT = 0.20;
    static normalizeDrawdownTo30Years(drawdown, simulationYears) {
    ...
    static computePearsonMatrix(samples) {
    ...
    static computeTailDependenceMatrix(samples, upper) {
        if (!Array.isArray(samples) || samples.length < 2)
            return [];
        const dimension = samples[0].length;
        if (dimension === 0)
    ...
    static computeMatrixMeanAbsoluteOffDiagonal(matrix) {
    ...
    static computeMatrixMaxAbsoluteOffDiagonal(matrix) {
        if (!Array.isArray(matrix) || matrix.length === 0)
            return 0;
        let max = 0;
        for (let row = 0; row < matrix.length; row += 1) {
    ...
    static normalizeScenarioMatrix(matrix) {
        if (!Array.isArray(matrix))
            return [];
        return matrix.map((row) => Array.isArray(row) ? row.map((value) => Number(value ?? 0)) : []);
    }
    static resolveScenarioTargetMatrix(scenario, paths, modelMatrices) {
        ...
        const scenarioPathCandidates = paths
            .map((path) => Array.isArray(path?.diagnostics?.targetCorrelation)
            ? path.diagnostics.targetCorrelation
            : Array.isArray(path?.correlationDiagnostics?.target)
                ? path.correlationDiagnostics.target
                : Array.isArray(path?.correlationDiagnostics?.byScenario?.[scenario]?.target)
                    ? path.correlationDiagnostics.byScenario[scenario].target
                    : [])
    ...
    static collectStableObservationSamples(paths) {
        const byScenario = {
            expansion: [],
            recession: [],
            stagflation: [],
        ...
        for (const path of sortedPaths) {
            const observationEntries = Array.isArray(path.__advancedObservationSamples) ? path.__advancedObservationSamples : [];
            for (let monthIndex = 0; monthIndex < observationEntries.length; monthIndex += 1) {
                const entry = observationEntries[monthIndex];
                const vector = Array.isArray(entry?.etfReturns) ? entry.etfReturns.map((value) => Number(value ?? 0)) : [];
                if (vector.length === 0)
                    continue;
                const scenario = entry?.scenario;
                if (!scenario || !byScenario[scenario])
                    continue;
                const canonicalEntry = { ...entry, simulationId: path.simulationId, monthIndex, scenario };
                const canonicalVector = canonicalEntry.etfReturns.map((value) => Number(value ?? 0));
    ...
    static createScenarioCorrelationSummary(scenario, samples, targetMatrix, sampleCount) {
    ...
    static createCanonicalCorrelationBundle(correlationDiagnostics) {
        ...
        const compatibility = {
            ...
            maeByScenario: correlationDiagnostics?.maeByScenario ?? Object.fromEntries(monte_carlo_model_1.MACRO_SCENARIOS.map((scenario) => [scenario, byScenario[scenario]?.meanAbsoluteDelta ?? 0])),
            rmseByScenario: correlationDiagnostics?.rmseByScenario ?? Object.fromEntries(monte_carlo_model_1.MACRO_SCENARIOS.map((scenario) => [scenario, 0])),
            maxAbsoluteErrorByScenario: correlationDiagnostics?.maxAbsoluteErrorByScenario ?? Object.fromEntries(monte_carlo_model_1.MACRO_SCENARIOS.map((scenario) => [scenario, byScenario[scenario]?.maxAbsoluteDelta ?? 0]))
        };
        return {
            overall: {
                ...overall,
    ...
    static hasMeaningfulCorrelationDiagnostics(correlationDiagnostics) {
        if (!correlationDiagnostics || typeof correlationDiagnostics !== 'object')
            return false;
        const matrixKeys = [
            'target',
            'operational',
        ...
        for (const key of matrixKeys) {
            const value = correlationDiagnostics[key];
        ...
        if (byScenario && typeof byScenario === 'object') {
            for (const scenarioDiagnostics of Object.values(byScenario)) {
                if (!scenarioDiagnostics || typeof scenarioDiagnostics !== 'object')
                    continue;
                for (const key of matrixKeys) {
                    const value = scenarioDiagnostics[key];
                    if (Array.isArray(value) && value.length > 0)
                        return true;
                }
            }
    ...
    static createRunLevelCorrelationDiagnostics(paths, modelMatrices, profileEvent) {
        ...
        const compatibility = {
            ...
            maeByScenario: Object.fromEntries(monte_carlo_model_1.MACRO_SCENARIOS.map((scenario) => [scenario, byScenario[scenario]?.meanAbsoluteDelta ?? 0])),
            rmseByScenario: Object.fromEntries(monte_carlo_model_1.MACRO_SCENARIOS.map((scenario) => [scenario, 0])),
            maxAbsoluteErrorByScenario: Object.fromEntries(monte_carlo_model_1.MACRO_SCENARIOS.map((scenario) => [scenario, byScenario[scenario]?.maxAbsoluteDelta ?? 0]))
        };
        return {
            overall,
            byScenario,
    ...
    static buildOfficialResult(paths, horizonYears, initialCapital, statisticsInput) {
        ...
        const pathCorrelationDiagnostics = advancedStatisticsEnabled ? paths.reduce((accumulator, path) => {
            const candidate = path?.correlationDiagnostics;
        ...
        const chosenCorrelationDiagnostics = advancedStatisticsEnabled
            ? (statisticsInput?.correlationDiagnostics ?? flatDiagnostics?.correlations
                ?? (this.hasMeaningfulCorrelationDiagnostics(pooledCorrelationDiagnostics) ? pooledCorrelationDiagnostics : (this.hasMeaningfulCorrelationDiagnostics(pathCorrelationDiagnostics) ? pathCorrelationDiagnostics : pooledCorrelationDiagnostics)))
            : {};
        const pathGeneralBenchmark = paths.reduce((accumulator, path) => {
            const candidate = path?.generalBenchmark;
            return accumulator ?? candidate;
        }, undefined);
        ...
        const officialReturnGeneration = officialStatistics?.returnGeneration ?? {};
        ...
        officialStatistics.advancedStatisticsEnabled = advancedStatisticsEnabled;
    ...
    static buildScenarioStatistics(paths, horizonYears, statisticsInput) {
        ...
        const returnGenerationAggregate = paths.reduce((aggregate, path) => {
            ...
            const candidateReturnCountFromRange = range.byEtfScenario
                ? Object.values(range.byEtfScenario).reduce((sum, value) => sum + (value?.candidateReturnCount ?? 0), 0)
                : 0;
        ...
        const macroSummary = {};
        for (const scenario of monte_carlo_model_1.MACRO_SCENARIOS) {
            const pathEpisodeDurations = [];
            const pathScenarioMonthIntensities = [];
            for (const path of paths) {
                ...
                let currentEpisodeScenario = null;
        ...
        const averageObservedMonthsPerScenario = observedEpisodeDurations.length > 0
            ...
            : (horizonYears * 12) / Math.max(1, monte_carlo_model_1.MACRO_SCENARIOS.length);
